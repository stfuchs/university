...
# -----------------------------------------
# pick_state - Removes and returns the state with the lowest
# cost function from open_states
#
def pick_state(open_states):
    f_min = open_states[0][0]
    f_min_idx = 0
    for i in range(len(open_states)):
        if (open_states[i][0] < f_min): 
            f_min = open_states[i][0]
            f_min_idx = i

    return open_states.pop(f_min_idx)
...
def plan(road, lane_change_cost, init, goal): # Don't change the name of this function!
    L = len(road) # Number of lanes
    S = len(road[0]) # Number of segments
    action = [[-1 for col in range(S)] for row in range(L)]
    expand = [[-1 for col in range(S)] for row in range(L)]
    costs = [[-1 for col in range(S)] for row in range(L)]
    heuristic = [[0 for col in range(S)] for row in range(L)]
    cur_cost = [0.0 for row in range(L)]
    for s in range(S):
        for l in range(L):
            x = s
            while (x >= 0 and road[l][S-1-x] == 0): x -= 1
            cur_cost[l] += 0.95 / road[l][S-1-x]
            heuristic[l][S-1-s] = cur_cost[l]

    motion = [[1,1], [1,0], [1,-1]]
    motion_name = ['\\','=','/']
    print_mat(heuristic)
    print
    l = init[0] # initial lane
    s = init[1] # initial segement
    g = 0 # current path cost
    h = 0 # heuristic
    f = g + h # 
    open_states = [[f, g, l, s]]
    closed_states = [[0 for col in range(S)] for row in range(L)]
    path = [['.' for col in range(S)] for row in range(L)]
    success = False
    count = 0
    
    while not success:
        if len(open_states) == 0: return 'fail'

        f, g, l, s = pick_state(open_states)
        if (s == goal[1] and l == goal[0]): 
            success = True
            cost = g
            break
        ...
        for i in range(len(motion)):
            l_n = l + motion[i][1]
            s_n = s + motion[i][0]
            if (l_n < 0 or l_n >= L or s_n >= S or road[l_n][s_n] == 0): continue
            g2 = g + 1.0 / road[l_n][s_n]
            if (motion[i][1] != 0): g2 += lane_change_cost
            open_states.append([g2 + heuristic[l_n][s_n], g2, l_n, s_n])
            closed_states[l_n][s_n] = 1
            action[l_n][s_n] = i
    ...
    print_mat(costs)
    path[goal[0]][goal[1]] = '*'
    while l != init[0] or s != init[1]:
        command = action[l][s]
        l = l - motion[command][1]
        s = s - motion[command][0]
        path[l][s] = motion_name[command]
...
test_road1 = build_road(8, [100, 10, 1], True)
...
#print plan(test_road4, lane_change_cost4, test_init4, test_goal4)
