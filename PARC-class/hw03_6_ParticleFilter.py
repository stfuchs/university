...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        turn = random.gauss(float(motion[0]), self.steering_noise)
        dist = float(motion[1]) + random.gauss(0.0, self.distance_noise)
        beta = dist / float(self.length) * tan(turn)
    ...
    def sense(self,noise=0): #do not change the name of this function
        ...
        if noise != 0:
            for i in range(len(landmarks)):
                bear = atan2(landmarks[i][0] - self.y, landmarks[i][1] - self.x) 
                Z.append((bear + random.gauss(.0, self.bearing_noise) - self.orientation) % (2*pi))
        else :
            for i in range(len(landmarks)):
                bear = atan2(landmarks[i][0] - self.y, landmarks[i][1] - self.x) 
                Z.append((bear - self.orientation) % (2*pi))
...
#print_measurements(measurements)
    ...
    if check_output(final_robot, estimated_position) == 1: f+=1
