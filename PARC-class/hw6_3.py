...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    expand_idx = [0,1]+range(4,dim+2)
    for k in range(len(data)):
        measurement = data[k][0]
        motion = data[k][1]
        Omega = Omega.expand(dim+2,dim+2,expand_idx)
        Xi = Xi.expand(dim+2,1,expand_idx,[0])
        ...
        Omega_prime = Omega.take(range(2,dim+2))
        Xi_prime = Xi.take(range(2,dim+2),[0])
        A = Omega.take([0,1],range(2,dim+2))
        B_inv = Omega.take([0,1]).inverse()
        C = Xi.take([0,1],[0])
        
        Omega = Omega_prime - A.transpose() * B_inv * A
        Xi = Xi_prime - A.transpose() * B_inv * C
